...
import { adjustPlanPrescription, applySessionCompletion, avoidPlanExercise, createManualWorkout, generateCategoryWorkout, generateWorkout, getAreaLoadBreakdown, removePlanExercise, reorderPlanExercise, scalePlanExercise, swapPlanExercise } from './domain/engine'
...
const titles: Record<View,string> = { home: 'Home', builder: 'Build a session', plan: 'Your session', library: 'Exercise library', player: 'Active session', progress: 'Progress', profile: 'Profile' }
...
export default function App() {
  ...
  else if (view === 'plan' && plan) content = <PlanScreen plan={plan} customExercises={state.customExercises} onStart={startPlan} onSave={() => setState(current => ({ ...current, savedPlans: [plan, ...current.savedPlans.filter(item => item.id !== plan.id)].slice(0,50) }))} onRegenerate={() => showPlan(lastPreferences ? generateWorkout(lastPreferences,state,`${Date.now()}`,plan.exercises.map(item=>item.exerciseId)) : generateCategoryWorkout('full_body',state), lastPreferences ?? undefined)} onEasier={index=>setPlan(scalePlanExercise(plan,index,-1,state))} onHarder={index=>setPlan(scalePlanExercise(plan,index,1,state))} onAdjust={(index,direction)=>setPlan(adjustPlanPrescription(plan,index,direction))} onSwap={index=>setPlan(swapPlanExercise(plan,index,state))} onReorder={(fromIndex,toIndex)=>setPlan(reorderPlanExercise(plan,fromIndex,toIndex))} onRemove={index=>setPlan(removePlanExercise(plan,index))} onAvoid={avoidFromPlan}/>
