...
import { recordProfileSignal, respondToProgression } from './domain/learning'
import './styles.css'
...
export default function App() {
  ...
  const avoidFromPlan = (index:number,id:string) => {
    const nextState=recordProfileSignal({...state,profile:{...state.profile,avoidList:[...new Set([...state.profile.avoidList,id])]}},id,'avoided',true)
  ...
  else if (view === 'progress') content = <ProgressScreen state={state} onProgression={(recommendation,response)=>setState(current=>respondToProgression(current,recommendation,response))}/>
