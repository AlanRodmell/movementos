...
export function ExerciseCard({ exercise, planned, compact = false, trailing }: { exercise: Exercise; planned?: WorkoutExercise; compact?: boolean; trailing?: React.ReactNode }) {
  return <article className={`exercise-card ${compact ? 'compact' : ''} ${planned?.adjusted ? 'adjusted' : ''}`}>
    ...
    <div className="exercise-copy">
      <div className="exercise-title-row"><h3>{exercise.name}</h3><span className="level-badge">L{exercise.level || '•'}</span>{planned?.adjusted&&<span className="adjusted-badge">Adjusted</span>}{planned?.setNumber&&<span className="set-badge">Set {planned.setNumber}/{planned.totalSets}</span>}</div>
