...
export function normaliseLearningModel(value:unknown):LearningModel{
  ...
  const learned=Object.fromEntries(Object.entries(rawExercises).filter(([id,item])=>/^[\w-]{1,100}$/.test(id)&&item&&typeof item==='object'&&!Array.isArray(item)).slice(0,1000).map(([id,item])=>{
    ...
    const progressionStatus=['none','ready','approaching','accepted','kept','deferred'].includes(String(entry.progressionStatus))?entry.progressionStatus as ExerciseLearningEntry['progressionStatus']:'none'
    const safeDate=(value:unknown)=>typeof value==='string'&&Number.isFinite(Date.parse(value))?value:null
    const safeId=(value:unknown)=>typeof value==='string'&&/^[\w-]{1,100}$/.test(value)?value:undefined
    const safePrescription=(value:unknown)=>typeof value==='string'?value.slice(0,80):undefined
    return[id,{...base,...entry,exposures:numeric('exposures'),completedAppearances:numeric('completedAppearances'),skips:numeric('skips'),swapsOut:numeric('swapsOut'),swapsIn:numeric('swapsIn'),positiveFeedback:numeric('positiveFeedback'),tooEasy:numeric('tooEasy'),tooHard:numeric('tooHard'),discomfort:numeric('discomfort'),negativePreference:numeric('negativePreference'),easierSelections:numeric('easierSelections'),harderSelections:numeric('harderSelections'),evidence:numeric('evidence'),successfulPerformances:numeric('successfulPerformances'),preference:clamp(Number(entry.preference)||0),difficultySuitability:clamp(Number(entry.difficultySuitability)||0),completionReliability:clamp(Number.isFinite(reliability)?reliability:.5,0,1),contexts,performanceHistory,lastDiscomfortAt,progressionStatus,progressionEvidenceAt:Math.max(0,Number(entry.progressionEvidenceAt)||0),lastSelectedAt:safeDate(entry.lastSelectedAt),lastCompletedAt:safeDate(entry.lastCompletedAt),currentExerciseId:safeId(entry.currentExerciseId),previousExerciseId:safeId(entry.previousExerciseId),currentPrescription:safePrescription(entry.currentPrescription),previousPrescription:safePrescription(entry.previousPrescription),deferredUntil:safeDate(entry.deferredUntil)??undefined}]
  }))
