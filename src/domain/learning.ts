...
export const emptyLearningEntry = (): ExerciseLearningEntry => ({
  ...
  progressionStatus:'none', progressionEvidenceAt:0,
})
...
export function normaliseLearningModel(value:unknown):LearningModel{
  ...
  const raw=value as Record<string,unknown>;const rawExercises=raw.exercises&&typeof raw.exercises==='object'&&!Array.isArray(raw.exercises)?raw.exercises as Record<string,unknown>:{}
...
function updateFeedback(entry:ExerciseLearningEntry, feedback:ExerciseFeedback, at:string, context?:LearningContext) {
  ...
  if (feedback === 'discomfort') next = { ...next, discomfort:next.discomfort+1, difficultySuitability:ema(next.difficultySuitability,-1,.42), completionReliability:ema(next.completionReliability,0,.28), evidence:next.evidence+3 }
...
export function applyLearningFromSession(model:LearningModel, session:WorkoutSession):LearningModel {
  ...
  rows.forEach((row,id)=>{
    ...
    const completed=row.completedAppearances??session.completedExerciseIds.filter(item=>item===id).length
    const success=completed>0&&!['too_hard','discomfort'].includes(row.feedback??'')&&['easy','good','unrated'].includes(session.rating)
    if(success)entry={...entry,successfulPerformances:entry.successfulPerformances+1,lastPerformance:row.performance??entry.lastPerformance}
    entries[id]=entry
  })
  if(session.rating!=='unrated')newEvents.push(event('overall_rating',`Workout rated ${session.rating}`,at))
  const routineKey=`${session.goal}|${session.intention}|${session.balanceReport?.templateKey??'legacy'}`;const previousRoutine=model.routineContexts[routineKey]??{confidence:0,positive:0,negative:0,evidence:0,lastUpdatedAt:at}
  ...
  const routine:RoutineLearningStat={confidence:session.rating==='unrated'?previousRoutine.confidence:ema(previousRoutine.confidence,target,.18),positive:previousRoutine.positive+(positiveRating?1:0),negative:previousRoutine.negative+(negativeRating?1:0),evidence:previousRoutine.evidence+(session.rating==='unrated'?0:1),lastUpdatedAt:at}
...
function recommendationFor(id:string,entry:ExerciseLearningEntry,state:Pick<AppState,'customExercises'|'profile'>,at:string):ProgressionRecommendation|null{
  ...
  if(!exercise||entry.successfulPerformances<3||entry.discomfort>0&&entry.lastCompletedAt&&Date.parse(at)-Date.parse(entry.lastCompletedAt)<30*86400000)return null
