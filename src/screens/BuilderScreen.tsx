...
export function BuilderScreen({ profile, dailyCheckIn, onCheckIn, onGenerate }: { profile: Profile; dailyCheckIn: DailyCheckIn; onCheckIn: (checkIn: DailyCheckIn) => void; onGenerate: (preferences: BuilderPreferences) => void }) {
  ...
  const [durationMinutes, setDuration] = useState<number | 'auto'>('auto')
  const [exercisesPerRound, setExercisesPerRound] = useState<number | 'auto'>('auto')
  const [targetSets, setTargetSets] = useState<number | 'auto'>('auto')
  const [recoveryModes, setRecoveryModes] = useState<RecoveryMode[]>(['mobility','stretching'])
  ...
  const next = () => {
    ...
    else onGenerate({ intention, goal: intention === 'recover' ? 'mobility' : goal, durationMinutes, focusAreas, equipment: selectedEquipment, level: profile.level, includeConditioning: intention === 'train' && includeConditioning, includeWarmup, exercisesPerRound, targetSets, recoveryModes })
  }
  return <div className="screen builder-screen">
    ...
    {step === 2 && <section className="wizard-panel"><span className="eyebrow">OUTCOME</span><h1>{intention === 'recover' ? 'How should we restore?' : 'What are you training for?'}</h1>{intention === 'train' ? <div className="choice-grid">{goals.filter(item => item.id !== 'mobility').map(item => <button key={item.id} className={goal === item.id ? 'mini-choice selected' : 'mini-choice'} onClick={() => { setGoal(item.id);if(item.id==='endurance')setIncludeConditioning(true) }}><strong>{item.title}</strong><small>{item.note}</small></button>)}</div> : <><p>Mobility develops controlled range. Stretching focuses on longer holds and tissue length. Select either or both.</p><div className="choice-grid"><button className={recoveryModes.includes('mobility')?'mini-choice selected':'mini-choice'} onClick={()=>toggleRecoveryMode('mobility')}><strong>Mobility</strong><small>Range, control and joint preparation</small></button><button className={recoveryModes.includes('stretching')?'mini-choice selected':'mini-choice'} onClick={()=>toggleRecoveryMode('stretching')}><strong>Stretching</strong><small>Longer holds for targeted areas</small></button></div><div className="fixed-meditation"><span>◌</span><div><strong>One meditation included</strong><small>Every generated session finishes with exactly one mindful exercise.</small></div></div></>}</section>}
    {step === 3 && <section className="wizard-panel"><span className="eyebrow">FOCUS</span><h1>Where should we focus?</h1><p>Select one or more specific areas, using the same choices as issues. Full body gives the engine maximum freedom.</p><button className={`nothing-flag ${focusAreas.includes('full_body')?'selected':''}`} onClick={() => setFocusAreas(['full_body'])}>Full body</button><BodyAreaPicker value={focusAreas.filter(area=>area!=='full_body')} onChange={areas=>setFocusAreas(areas.length?areas:['full_body'])}/></section>}
    {step === 4 && <section className="wizard-panel"><span className="eyebrow">CONSTRAINTS</span><h1>Shape the session</h1><h3>Workout time</h3><div className="filter-pills">{(intention==='recover'?[5,10,15,20,30]:['auto',15,20,30,45] as Array<number|'auto'>).map(value=><button key={value} className={durationMinutes===value?'selected':''} onClick={()=>setDuration(value as number|'auto')}>{value==='auto'?'Auto':`${value} min`}</button>)}</div>{intention==='train'&&<><h3 className="constraint-heading">Exercises per round</h3><div className="filter-pills">{(['auto',3,4,5,6,8] as Array<number|'auto'>).map(value=><button key={value} className={exercisesPerRound===value?'selected':''} onClick={()=>setExercisesPerRound(value)}>{value==='auto'?'Auto':value}</button>)}</div><h3 className="constraint-heading">Target sets</h3><div className="filter-pills">{(['auto',2,3,4,5,6] as Array<number|'auto'>).map(value=><button key={value} className={targetSets===value?'selected':''} onClick={()=>setTargetSets(value)}>{value==='auto'?'Auto (max 3)':value}</button>)}</div></>}<div className="builder-toggles"><label className="toggle-row"><span><strong>Preparation warm-up</strong><small>{intention==='recover'?'Optional for mobility or stretching sessions':'Prepare the selected areas before the main work'}</small></span><input type="checkbox" checked={includeWarmup} onChange={event=>setIncludeWarmup(event.target.checked)}/></label>{intention==='train'&&<label className="toggle-row"><span><strong>Conditioning block</strong><small>Add a short capacity finisher</small></span><input type="checkbox" checked={includeConditioning} onChange={event=>setIncludeConditioning(event.target.checked)}/></label>}</div><h3>Available today</h3><div className="filter-pills">{equipment.map(([id,label]) => <button key={id} className={selectedEquipment.includes(id) ? 'selected' : ''} onClick={() => toggleEquipment(id)}>{label}</button>)}</div></section>}
