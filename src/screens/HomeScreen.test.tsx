...
const callbacks=()=>({onTrainingEffort:vi.fn(),onBuild:vi.fn(),onSuggested:vi.fn(),onCategory:vi.fn(),onResume:vi.fn(),onOpenPlan:vi.fn(),onViewSaved:vi.fn()})
...
it('shows the empty personalised launch state and routes every quick start',()=>{
  ...
  const effortSlider=screen.getByRole('slider',{name:'Training effort'})
  expect(effortSlider).toHaveValue('1')
  expect(effortSlider).toHaveAttribute('aria-valuetext',expect.stringMatching(/^Standard/))
  fireEvent.change(effortSlider,{target:{value:'2'}})
  expect(actions.onTrainingEffort).toHaveBeenCalledWith('push')
