...
export function LibraryScreen({ state, onToggleFavourite, onToggleAvoid }: { state: AppState; onToggleFavourite: (id: string) => void; onToggleAvoid: (id: string) => void }) {
  ...
  return <div className="screen library-screen">
    <section className="library-header"><span className="eyebrow">EXERCISE CATALOGUE</span><h1>Explore {allExercises.length} movements</h1><p>Search by movement, muscle, pattern, or filter by the kit you have.</p><input className="search-input" type="search" placeholder="Search exercises…" value={search} onChange={event => setSearch(event.target.value)}/></section>
