...
import type { ActiveSession, Exercise, WorkoutSession } from '../domain/types'

const formatTime = (seconds: number) => `${Math.floor(seconds / 60)}:${String(seconds % 60).padStart(2,'0')}`

export function PlayerScreen({ session, customExercises, soundEnabled, onProgress, onComplete, onExit }: { session: ActiveSession; customExercises: Exercise[]; soundEnabled: boolean; onProgress: (session: ActiveSession) => void; onComplete: (session: WorkoutSession) => void; onExit: () => void }) {
  ...
  const advance = (log: boolean) => {
    ...
    setIndex(next); setRemaining(duration); deadline.current = Date.now() + duration * 1000; setRunning(true)
  }
  const finish = (rating: WorkoutSession['rating']) => onComplete({
    ...
    completedExerciseIds: completed, exercises: completed.map(id => { const item = plan.exercises.find(entry => entry.exerciseId === id); const resolved = exerciseById.get(id) ?? customExercises.find(entry => entry.id === id); return { id, name: resolved?.name ?? id, prescription: item?.prescription ?? '', durationSeconds: item?.durationSeconds ?? 0 } }),
  })
