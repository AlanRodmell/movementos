...
import type { ActiveSession, AppState, Category, Exercise, ExerciseFeedback, ExercisePerformance, Goal, LearningContext, SessionExercise, WorkoutAction, WorkoutActionType, WorkoutExercise, WorkoutSession } from '../domain/types'
...
export function PlayerScreen({ session, state, customExercises, soundEnabled, waitBetweenExercises, areaLoadBefore, onProgress, onComplete, onExit }: { session: ActiveSession; state:AppState; customExercises: Exercise[]; soundEnabled: boolean; waitBetweenExercises:boolean; areaLoadBefore: Partial<Record<Category,number>>; onProgress: (session: ActiveSession) => void; onComplete: (session: WorkoutSession) => void; onExit: () => void }) {
  ...
  if(finished&&!ratingStage)return <div className="player-screen workout-review"><span className="eyebrow">SESSION REVIEW</span><h1>How did each movement fit?</h1><p>Optional feedback helps tune future sessions. Repeated sets are grouped together.</p><div className="review-list">{reviewRows().map(row=><section className="review-row" key={row.id}><header><div><strong>{row.name}</strong><small>{row.prescription} · {row.completedAppearances}/{row.plannedAppearances} completed{row.skippedAppearances?` · ${row.skippedAppearances} skipped`:''}{row.swapped?' · swapped':''}{row.adjusted?' · adjusted':''}</small></div></header><div className="feedback-chips">{([['good_fit','Good fit'],['too_easy','Too easy'],['too_hard','Too hard'],['discomfort','Discomfort'],['didnt_enjoy',"Didn't enjoy"]] as const).map(([value,label])=><button className={feedback[row.id]===value?'selected':''} key={value} onClick={()=>setFeedback(values=>({...values,[row.id]:values[row.id]===value?undefined:value}))}>{label}</button>)}</div><details><summary>Record achieved reps, time or load</summary><div className="performance-inputs"><input aria-label={`${row.name} achieved reps`} inputMode="numeric" placeholder="Reps" onChange={event=>setPerformance(values=>({...values,[row.id]:{...values[row.id],achievedReps:Number(event.target.value)||undefined}}))}/><input aria-label={`${row.name} achieved seconds`} inputMode="numeric" placeholder="Seconds" onChange={event=>setPerformance(values=>({...values,[row.id]:{...values[row.id],achievedSeconds:Number(event.target.value)||undefined}}))}/><input aria-label={`${row.name} load`} inputMode="decimal" placeholder="Load" onChange={event=>setPerformance(values=>({...values,[row.id]:{...values[row.id],load:Number(event.target.value)||undefined}}))}/><select aria-label={`${row.name} load unit`} onChange={event=>setPerformance(values=>({...values,[row.id]:{...values[row.id],loadUnit:event.target.value as 'kg'|'lbs'}}))}><option value="kg">kg</option><option value="lbs">lbs</option></select></div></details></section>)}</div><button className="primary review-continue" onClick={()=>setRatingStage(true)}>Continue to overall rating <span>→</span></button></div>
