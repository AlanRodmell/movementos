...
function migrateHistory(raw: Record<string, unknown>): WorkoutSession[] {
  ...
  return source.filter(item => item && typeof item === 'object').slice(0, 250).map((item, index) => {
    ...
    return {
      ...
      planStructure:session.planStructure&&typeof session.planStructure==='object'?(()=>{const structure=session.planStructure as Record<string,unknown>;return{targetDurationMinutes:Math.max(1,Number(structure.targetDurationMinutes)||1),mainExerciseCount:Math.max(0,Number(structure.mainExerciseCount)||0),totalSets:Math.max(0,Number(structure.totalSets)||0),includeWarmup:Boolean(structure.includeWarmup),includeConditioning:Boolean(structure.includeConditioning)}})():undefined,
    }
  })
}
...
function rebuildExerciseStats(history:WorkoutSession[]):Record<string,ExerciseStat> {
  ...
  history.slice().sort((a,b)=>Date.parse(a.date)-Date.parse(b.date)).forEach(session=>[...new Set(session.completedExerciseIds)].forEach(id=>{
    ...
    const row=session.exercises.find(item=>item.id===id);const individualRating:WorkoutSession['rating']=row?.feedback==='good_fit'?'good':row?.feedback==='too_easy'?'easy':row?.feedback==='too_hard'?'hard':row?.feedback==='discomfort'?'brutal':row?.feedback==='didnt_enjoy'?'unrated':session.rating
    const positive=individualRating==='easy'||individualRating==='good';const consecutiveSuccesses=positive?previous.consecutiveSuccesses+1:0
    stats[id]={ attempts:previous.attempts+1,completed:previous.completed+1,easyGood:previous.easyGood+(positive?1:0),hard:previous.hard+(individualRating==='hard'?1:0),brutal:previous.brutal+(individualRating==='brutal'?1:0),consecutiveSuccesses,lastRating:individualRating,lastCompletedAt:session.date,lastDurationSeconds:row?.durationSeconds??null,progressionReady:consecutiveSuccesses>=3,coachDecision:individualRating==='brutal'?'regress':consecutiveSuccesses>=3?'progress':'maintain' }
  }))
  return stats
}
...
export function normaliseState(value: unknown): AppState {
  ...
  return {
    schemaVersion: SCHEMA_VERSION,
    // Existing v11 installs pre-date onboarding and should not be interrupted.
...
export function serializeLegacyState(state: AppState) {
  ...
  return {
    schemaVersion: SCHEMA_VERSION, onboardingCompleted:state.onboardingCompleted, trainingEffort:state.trainingEffort, trainingEffortDate:state.trainingEffortDate, dailyCheckIn:state.dailyCheckIn,
